...
> docker 支持
    ...
    example:
        # 启动 web 服务
        docker run jyolo/wlogger:v1.3 -r web -c config.ini  #  需要把配置文件copy 进 docker /wLogger 目录内
        
        # 启动 解析存储器 服务
        docker run jyolo/wlogger:v1.3 -r outputer -c config.ini  #  需要把配置文件copy 进 docker /wLogger 目录内
